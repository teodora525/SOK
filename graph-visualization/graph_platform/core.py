"""
Platform - jezgro aplikacije sa Singleton Pattern
...
class PluginManager:
    ...
    def register_data_source_plugin(self, plugin_name: str, plugin: DataSourcePlugin) -> None:
        """Registruj Data Source plugin - sprečava duplikate"""
        if plugin_name not in self.data_source_plugins:
            self.data_source_plugins[plugin_name] = plugin
            print(f"✓ Registered data source plugin: {plugin_name}")
        else:
    ...
    def register_visualizer_plugin(self, plugin_name: str, plugin: VisualizerPlugin) -> None:
        """Registruj Visualizer plugin - sprečava duplikate"""
        if plugin_name not in self.visualizer_plugins:
            self.visualizer_plugins[plugin_name] = plugin
            print(f"✓ Registered visualizer plugin: {plugin_name}")
        else:
            print(f"⚠ Plugin {plugin_name} already registered, skipping")
...
class SingletonMeta(type):
    """
    Metaclass za implementaciju Singleton pattern-a.
        ...
        if cls not in cls._instances:
            ...
            with cls._lock:
                if cls not in cls._instances:
                    ...
                    print(f"✓ Created new {cls.__name__} singleton instance")
        else:
...
class Platform(metaclass=SingletonMeta):
    ...
    Implementirana kao Singleton - samo jedna instanca u celoj aplikaciji.
    """
    ...
    def reset_singleton(cls):
        """Reset singleton instance - korisno za testiranje"""
        if cls in cls._instances:
            del cls._instances[cls]
            print("✓ Platform singleton reset")